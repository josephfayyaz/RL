...
def evaluate_agent_on_env(env, agent, episodes, success_threshold):
    """
    Evaluates the trained agent on the given environment for a fixed number of episodes.
    Logs mean, std, 5th percentile, and success rate (how often return ≥ threshold).
    """
    returns = []
    for _ in range(episodes):
        ...
        while not done:
            action, _ = agent.get_action(state, evaluation=True)  # Greedy action during evaluation
    ...
    p5_r = np.percentile(returns, 5)  # Robustness measure
    ...
    with open(LOG_CSV_PATH, "w", newline="") as training_csv:
        ...
        while total_timesteps < config["total_timesteps"]:
            ...
            if done:
                ...
                # Log first time return ≥ success threshold (e.g. 1000)
