...
# --- Configuration ---
data_dir         = r".\Logs\metricextraction"
...
for algo in sorted(os.listdir(data_dir)):
    ...
    for seed in seeds:
        ...
        efile = next((f for f in files if f"seed_{seed}" in f and "5000000" in f), None)
...
for ax, metric in zip(axes, metrics):
    ...
    sns.barplot(
        ...
        ax=ax,
        dodge=True,
        palette=sns.color_palette(["#4477AA", "#66A1D2", "#2DC6DA"])  # One color per seed
...
# Reset index to make it clean and easy to read
df_final = df_raw.reset_index()

# Print to console (optional)
print("\n===== Final Metrics per Algorithm and Seed =====")
print(df_final)

# Save to CSV
csv_output_path = os.path.join(savepath, "all_metrics_by_seed.csv")
df_final.to_csv(csv_output_path, index=False)
print(f"\n✅ Metrics saved to: {csv_output_path}")