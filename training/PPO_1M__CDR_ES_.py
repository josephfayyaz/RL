...
def train_agent(algo, env_id, eval_env_id, USE_entropy_scheduler, total_timesteps, save_path, log_path,seed,csv_filename):
    ...
    learning_curve_cb = LearningCurveCallback(
        eval_env=eval_env,
        csv_path=f"logs/learning_curve_{algo}_Domain _{args.Domain}_ES_{args.Entropy_Scheduling}_seed_{seed}.csv",
        eval_interval=5000,
        n_eval_episodes=5,
        verbose=1
        )
    ...
    modelpath=os.path.join(save_path, f"{algo}_Domain_{args.Domain}_ES_{args.Entropy_Scheduling}_seed_{seed}_({env_id}_{eval_env_id})")
